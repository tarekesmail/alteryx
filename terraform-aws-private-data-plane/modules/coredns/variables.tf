...
variable "COREDNS_AWS_AMI_SHARING_CREDS" {
...
variable "COREDNS_AWS_AMI_NAME_PREFIX_REGEX" {
...
variable "COREDNS_AWS_AMI_CURRENT_VERSION" {
  ...
  validation {
    ...
    condition     = try(var.COREDNS_AWS_AMI_CURRENT_VERSION == "latest", false) || can(regex("^(0|[1-9]\\d*)\\.(0|[1-9]\\d*)\\.(0|[1-9]\\d*)(?:-((?:0|[1-9]\\d*|\\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\\.(?:0|[1-9]\\d*|\\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\\+([0-9a-zA-Z-]+(?:\\.[0-9a-zA-Z-]+)*))?$", var.COREDNS_AWS_AMI_CURRENT_VERSION))
  }
}
